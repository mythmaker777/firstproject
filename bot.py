"""
SG Tuition Match Bot — Final Version
======================================
Features:
  - Tutor registration (free)
  - Monthly subscription model (PayNow → screenshot → admin approves)
  - Subscribed tutors: unlimited job alerts + instant contact reveals
  - Parents: always free, can report tutors
  - Admin: approve/reject subscriptions, list/ban/delete tutors and jobs, view stats
  - Auto-ban on 3 reports, expiry warnings 3 days before renewal
  - Browse gated by registration and subscription status
"""
...
SUBSCRIPTION_FEE = int(os.environ.get("SUBSCRIPTION_FEE", "20"))

# Number of reports before a tutor is automatically banned
...
# Subscription payment screenshot (18)
...
async def how_it_works_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.callback_query.message.reply_text(
        ...
        f"4. Express interest — parent's contact revealed immediately\n"
        f"5. Unlimited matches for the month 🎉\n\n"
        f"No agency commissions. No per-match fees. Just a flat monthly subscription.",
        parse_mode="Markdown",
    )
...
async def subscribe_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Inform if already active but still allow early renewal
    ...
    if sub:
        ...
        await update.message.reply_text(
            ...
            f"You can renew early — your new 30 days will start from today.",
            parse_mode="Markdown",
        )
...
async def browse_jobs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not tutor:
        await send(
            "👤 *Register first to view tuition jobs.*\n\n"
            "Use /register to set up your tutor profile — it only takes a few minutes!",
            parse_mode="Markdown",
        )
        return
    ...
    if not sub:
        ...
        if pending:
            await send(
                ...
                "You'll be able to browse and apply for jobs as soon as it's activated.\n"
                "This usually takes a few hours.",
                parse_mode="Markdown",
            )
        else:
            await send(
                f"🔒 *Subscribe to view and apply for tuition jobs.*\n\n"
                f"For just *${SUBSCRIPTION_FEE}/month* you get:\n"
                "✅ Instant alerts for matching jobs\n"
                "✅ Unlimited contact reveals — no per-match fees\n"
...
async def express_interest(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Subscribed tutors get the parent's contact instantly. No payment wall."""
    query = update.callback_query
    await query.answer()
    ...
    # Send contact to tutor immediately
    await query.message.reply_text(
        f"✅ *You're connected!*\n\n"
        f"*Job details:*\n{job_summary(job)}"
    ...
    # Notify parent with a report button
    try:
        report_keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton(
                "🚩 Report This Tutor",
                callback_data=f"report_{tutor['id']}_{job_id}"
            )]
        ])
        await context.bot.send_message(
    ...
    if sub:
        ...
        if pending:
            await update.message.reply_text(
                ...
                "Your screenshot has been received and is being reviewed. "
...
async def report_tutor_reasons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Parent taps Report — show reason options. callback: report_{tutor_id}_{job_id}"""
    query = update.callback_query
    await query.answer()
...
async def report_tutor_submit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Parent selects reason — saves report, notifies admin, auto-bans at threshold."""
    query = update.callback_query
    await query.answer()
    ...
    if ADMIN_ID:
        auto_ban_note = "\n\n⚠️ *TUTOR HAS BEEN AUTO-BANNED* (reached report threshold)" if auto_banned else ""
        admin_keyboard = InlineKeyboardMarkup([
            [
        ...
        await context.bot.send_message(
            ...
            text=(
                f"🚩 *New Tutor Report*\n\n"
                f"Report #{report_id} | Total reports on this tutor: {report_count}\n\n"
                f"*Reported Tutor:*\n{tutor_summary(tutor)}"
                f"📱 Phone: {tutor['phone']}\n"
                f"🆔 TG ID: `{tutor['telegram_id']}`\n\n"
                f"*Reason:* {reason}\n\n"
...
async def admin_dismiss_report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text(f"✅ Report #{report_id} dismissed. No action taken.")
...
async def list_tutors(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for t in tutors:
        sub = db.get_active_subscription(t["id"])
        if sub:
            remaining = days_remaining(sub["expires_at"])
            sub_status = f"🟢 Active ({remaining}d left)"
        else:
            pending = db.get_pending_subscription(t["id"])
            sub_status = "⏳ Pending approval" if pending else "🔴 No subscription"
...
async def admin_delete_tutor(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text(
        f"⚠️ Are you sure you want to delete *{tutor['name']}*?\n\n"
        "This will remove their profile and all records. This cannot be undone.",
        parse_mode="Markdown",
        reply_markup=keyboard,
    )
...
async def admin_confirm_delete_tutor(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(
            chat_id=tutor["telegram_id"],
            text=(
                "Your tutor profile has been removed from SG Tuition Match.\n\n"
                "If you believe this is a mistake, please contact us."
            ),
        )
    except Exception:
    ...
    await query.message.reply_text(
        f"✅ Tutor *{tutor['name']}* (#{tutor_id}) has been deleted.",
        parse_mode="Markdown",
    )
...
async def admin_ban_tutor(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(
            chat_id=tutor["telegram_id"],
            text=(
                "⚠️ Your tutor profile has been suspended from SG Tuition Match.\n\n"
    ...
    await query.message.reply_text(
        f"🚫 Tutor *{tutor['name']}* has been banned.",
        parse_mode="Markdown",
    )
...
async def check_expiring_subscriptions(context: ContextTypes.DEFAULT_TYPE):
    """Runs daily. Warns tutors 3 days before expiry and expires overdue subscriptions."""
...
def main():
    ...
    # Daily expiry check at midnight SGT (16:00 UTC)
