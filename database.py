"""
database.py — SQLite wrapper with subscription model
"""
...
class Database:
    ...
    def find_matching_subscribed_tutors(self, subject, level, zone, budget) -> list:
        """
        Like find_matching_tutors but ONLY returns tutors with an active subscription.
        This is used when notifying tutors of new jobs — unsubscribed tutors
        don't get alerts, which is the main incentive to subscribe.
        """
    ...
    def create_subscription(self, tutor_id, amount) -> int:
        """Creates a pending subscription record. Returns subscription ID."""
        with self._conn() as conn:
            cursor = conn.execute(
    ...
    def get_active_subscription(self, tutor_id) -> Optional[dict]:
        """Returns the tutor's current active subscription if it hasn't expired."""
        now = datetime.utcnow().isoformat()
        with self._conn() as conn:
            row = conn.execute(
                """
    ...
    def get_pending_subscription(self, tutor_id) -> Optional[dict]:
        """Returns a pending subscription (screenshot sent, awaiting approval)."""
        with self._conn() as conn:
            row = conn.execute(
                """
                SELECT * FROM subscriptions
    ...
    def activate_subscription(self, sub_id) -> str:
        """Marks subscription as active for 30 days. Returns the expiry datetime string."""
    ...
    def get_subscriptions_expiring_in_days(self, days: int) -> list:
        """Returns active subscriptions expiring within the next `days` days."""
        now = datetime.utcnow()
    ...
    def expire_old_subscriptions(self) -> int:
        """Marks all past-expiry active subscriptions as expired. Returns count."""
        now = datetime.utcnow().isoformat()
        with self._conn() as conn:
            cursor = conn.execute(
                """
    ...
    def get_stats(self) -> dict:
        with self._conn() as conn:
            now = datetime.utcnow().isoformat()
            tutors = conn.execute(
                "SELECT COUNT(*) FROM tutors WHERE active = 1"
            ).fetchone()[0]
            ...
            total_interests = conn.execute(
                "SELECT COUNT(*) FROM interests"
            ).fetchone()[0]
            total_earned = conn.execute(
                "SELECT COALESCE(SUM(amount), 0) FROM subscriptions WHERE status = 'active' OR status = 'expired'"
            ).fetchone()[0]
        return {
            "tutors": tutors,
